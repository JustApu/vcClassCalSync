	...
	$selectEvent->fetchrow_array())
{
  ...
  else
  {
    ...
    {
      # Update curEvents with the Google calendar event ID
      my $insertEvent = $dbh->prepare("INSERT into curEvents
	...
	$description, $room, $googleID);
      ...
      # Delete event from toAdd table
      my $deleteEvent = $dbh->prepare("DELETE from toAdd WHERE eventID = ?");
