	...
	$selectEvent->fetchrow_array())
{
  ...
  else
  {
    ...
    {
      # Update curEvents
      my $updateEvent = $dbh->prepare("UPDATE curEventsAthl
	...
	WHERE `eventID` = ? AND `googleID` = ?");
      ...
      # Delete event from toUpdateAthl table
      my $deleteEvent = $dbh->prepare("DELETE from toUpdateAthl WHERE eventID = ?");
